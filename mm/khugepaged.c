...
static unsigned long khugepaged_collapse_pud __read_mostly = 0;
static unsigned long khugepaged_collapse_pmd __read_mostly = 0;
/* during fragmentation poll the hugepage allocator once every minute */
//static unsigned int khugepaged_alloc_sleep_millisecs __read_mostly = 60000;
static unsigned int khugepaged_alloc_sleep_millisecs __read_mostly = 10000;
...
static ssize_t khugepaged_collapse_pud_show(struct kobject *kobj,
		struct kobj_attribute *attr,
		char *buf)
{
	return sprintf(buf, "%ld\n", khugepaged_collapse_pud);
}

static ssize_t khugepaged_collapse_pud_store(struct kobject *kobj,
		struct kobj_attribute *attr,
		const char *buf, size_t count)
{
	int err;
	...
	khugepaged_collapse_pud = enable;

	return count;
}
static struct kobj_attribute khugepaged_collapse_pud_attr =
__ATTR(khugepaged_collapse_pud, 0644, khugepaged_collapse_pud_show,
		khugepaged_collapse_pud_store);

static ssize_t khugepaged_collapse_pmd_show(struct kobject *kobj,
		struct kobj_attribute *attr,
		char *buf)
{
	return sprintf(buf, "%ld\n", khugepaged_collapse_pmd);
}

static ssize_t khugepaged_collapse_pmd_store(struct kobject *kobj,
		struct kobj_attribute *attr,
		const char *buf, size_t count)
{
	int err;
	...
	khugepaged_collapse_pmd = enable;

	return count;
}
static struct kobj_attribute khugepaged_collapse_pmd_attr =
__ATTR(khugepaged_collapse_pmd, 0644, khugepaged_collapse_pmd_show,
		khugepaged_collapse_pmd_store);
...
/* Trident provides a configurable interface to khugepaged via the sysfs for
 * selection of page size to collapse. To enable promotion to 1GB pages, do
 * echo 1 > /sys/kernel/mm/transparent_hugepage/khugepaged/khugepaged_pud.
 * Similarly, to enable promotion into 2MB page, do
 * echo 1 > /sys/kernel/mm/transparent_hugepage/khugepaged/khugepaged_pmd.
 */
...
/* This is a helper function which actually tells the page size.  -1 means
 * invalid address, 4 means base page, 2 means 2MB page, 1 means 1GB page.
 */
static int is_address_huge(struct mm_struct *mm, unsigned long address)
{
	pgd_t *pgd;
	p4d_t *p4d;
	pud_t *pud;
...
static void release_pmd_page(pmd_t *pmd){
	struct page *page;
	page = pmd_page(*pmd);
...
static int __collapse_huge_pmd_isolate(struct vm_area_struct *vma,
		unsigned long address,
		pmd_t *pmd)
{
	struct page *page = NULL;
	int result = 0, referenced = 0;
	bool writable = false;
...
static void __collapse_huge_pmd_copy(pmd_t *pmd, struct page *page,
		...
		spinlock_t *pmd_ptl)
{
	...
	copy_user_huge_page(page, src_page, address, vma, HPAGE_PMD_NR);
...
/* This method allocates a huge 1GB page */
	static struct page *
khugepaged_alloc_huge_onegb_page(struct page **hpage, gfp_t gfp, int node)
{
	VM_BUG_ON_PAGE(*hpage, *hpage);

	*hpage = __alloc_pages_node(node, gfp, HPAGE_PUD_ORDER);
	if (unlikely(!*hpage)) {
		count_vm_event(THHP_COLLAPSE_ALLOC_FAILED);
		*hpage = ERR_PTR(-ENOMEM);
		return NULL;
	}

	prep_transhuge_page(*hpage);
	count_vm_event(THHP_COLLAPSE_ALLOC);
	return *hpage;
}
...
static int huge_onegb_page_vma_revalidate(struct mm_struct *mm, unsigned long address,
		struct vm_area_struct **vmap)
{
	struct vm_area_struct *vma;
	unsigned long hstart, hend;
	...
	*vmap = vma = find_vma(mm, address);
	...
	hstart = (vma->vm_start + ~HPAGE_PUD_MASK) & HPAGE_PUD_MASK;
	hend = vma->vm_end & HPAGE_PUD_MASK;
	if (address < hstart || address + HPAGE_PUD_SIZE > hend)
		return SCAN_ADDRESS_RANGE;
	if (!hugepage_vma_check(vma))
		return SCAN_VMA_CHECK;
	return 0;
		...
		int node, int referenced)
{
	...
	pte_t *pte, *_pte;
	struct page *new_page;
	spinlock_t *pmd_ptl, *pte_ptl, *pud_ptl;
	int isolated = 0, result = 0;
	struct mem_cgroup *memcg;
	struct vm_area_struct *vma;
	unsigned long mmun_start;	/* For mmu_notifiers */
	...
	int loop_index = 0;
	unsigned long backed_address = address; //Have a backup, may not be used.
	...
	result = huge_onegb_page_vma_revalidate(mm, address, &vma);
	if (result) {
		mem_cgroup_cancel_charge(new_page, memcg, true);
		up_read(&mm->mmap_sem);
		goto out_nolock;
	...
	/* check if the pud and pmd is still valid */
	if ((mm_find_pud(mm, address) != pud))
		goto out;
	anon_vma_lock_write(vma->anon_vma);

	mmun_start = address;
	mmun_end = address + HPAGE_PUD_SIZE;
	mmu_notifier_invalidate_range_start(mm, mmun_start, mmun_end);
	pud_ptl = pud_lock(mm, pud);	/*probably unnecessary */

	_pud = pudp_collapse_flush(vma, address, pud);
	spin_unlock(pud_ptl);
	mmu_notifier_invalidate_range_end(mm, mmun_start, mmun_end);
	...
	for(loop_index = 0; loop_index < HPAGE_PMD_NR; loop_index++){
		...
		if(pmd_trans_huge(*pmd)){
			...
			spin_lock(pmd_ptl);
			isolated = __collapse_huge_pmd_isolate(vma, address, pmd);
			spin_unlock(pmd_ptl);
		}
		else{
			pte = pte_offset_map(pmd, address);
		...
		//printk(KERN_DEBUG "DEBUG: Passed %s %d \n",__FUNCTION__,__LINE__);
		if(unlikely(!isolated)){
			/* This means that there is some problem in the isolation.
			 * So the pte's of this pmd are already reverted by the
			 * __collapse_huge_page_isolate function. But, we need to revert the
			 * pte's of the previous pmd's under the pud
			 */
			pte_unmap(pte);
			while(address != backed_address){
				address -= PAGE_SIZE * HPAGE_PMD_NR;
				pmd = pmd_offset(&_pud, address);
				if(pmd_none(*pmd)){
					continue;
			...
			BUG_ON(!pud_none(*pud));
			/*
			 * We can only use set_pud_at when establishing
			 * hugepuds and never for establishing regular puds that
			 * points to regular pagetables. Use pud_populate for that
			 */
			pmd = pmd_offset(&_pud, address);
	...
	//printk(KERN_DEBUG "DEBUG: Passed %s %d \n",__FUNCTION__,__LINE__);
	/* If we reach here, we successfully isolated the entire 1GB range of pud.
	 * We can proceed further deterministically.
	 */
	/*
	 * All pages are isolated and locked so anon_vma rmap
	 * can't run anymore.
	...
	__SetPageUptodate(new_page);
	_pud_orig = _pud;
	_pud = mk_huge_pud(new_page, vma->vm_page_prot);
	_pud = maybe_pud_mkwrite(pud_mkdirty(_pud), vma);


	/* We must free all the pgtables, and the pud */
	for(loop_index = 0; loop_index < HPAGE_PMD_NR; loop_index++){
		pmd = pmd_offset(&_pud_orig, address);
		if(pmd_none(*pmd)){
			address += PAGE_SIZE * HPAGE_PMD_NR;
			continue;
		}
		if(!pmd_trans_huge(*pmd)){
			pte_free(vma->vm_mm, pmd_pgtable(*pmd));
			mm_dec_nr_ptes(vma->vm_mm);
		}
		address += PAGE_SIZE * HPAGE_PMD_NR;
	}
	pte_free(vma->vm_mm, pud_pmdtable(_pud_orig));
	...
	mem_cgroup_commit_charge(new_page, memcg, false, true);
	lru_cache_add_active_or_unevictable(new_page, vma);
	set_pud_at(mm, address, pud, _pud);
...
static int khugepaged_scan_pud(struct mm_struct *mm,
		struct vm_area_struct *vma,
		unsigned long address,
		struct page **hpage)
{
	pmd_t *pmd;//We will be requiring this also.
	pud_t *pud;
	pte_t *pte, *_pte;
	int ret = 0, none_or_zero = 0, result = 0, referenced = 0;
	struct page *page = NULL;
	unsigned long _address, temp_address;
	spinlock_t *ptl;
	...
	memset(khugepaged_node_load, 0, sizeof(khugepaged_node_load));
	for(loop_index = 0; loop_index < HPAGE_PMD_NR; loop_index++){
		/* The modified code starts from here */
		pmd = mm_find_pmd(mm, address);
		if(!pmd) {
			if(is_address_huge(mm, address) == 2)
				pmd = pmd_offset(pud, address);
			else{
				address += PAGE_SIZE * HPAGE_PMD_NR;
				continue;
		...
		//We are going to the next pmd now. So increment the address.
		address = _address;
		pte_unmap_unlock(pte, ptl);
	}
	if (writable) {
...
skip:
		...
		while (khugepaged_scan.address < hend) {
			...
			if (shmem_file(vma->vm_file)) {
				...
				/* Right now, we extend the khugepaged to service the 1GB anonymous
				 * pages, hence we don't care about shared pages
				 */
				/* We have to be careful, because we only scan the 1GB if it is
				 * physically aligned, and also it is within the ranges of the vma.
				 */
			...
			/* move to next address, depending on whether it is a pud, or pmd */
			if(!is_pud){
				khugepaged_scan.address += HPAGE_PMD_SIZE;
				progress += HPAGE_PMD_NR;
			}
			else{
				khugepaged_scan.address += HPAGE_PUD_SIZE;
				progress += HPAGE_PUD_NR;
			}
			if (ret)
